...
$report_RET = DBGet( "SELECT ID,TITLE,PHP_SELF,SEARCH_PHP_SELF,SEARCH_VARS
	FROM saved_reports WHERE ID='" . (int) $_REQUEST['id'] . "'" );
...
else
{
	...
	echo '<script>
		...
		report_link.href = ' . json_encode( $report['PHP_SELF'] ) . ';
