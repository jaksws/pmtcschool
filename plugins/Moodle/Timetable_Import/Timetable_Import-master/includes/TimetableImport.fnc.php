 ...
 * @uses PHPExcel class
 *
 * @param string $course_periods_import_file_path Excel file path.
 *
 * @return string CSV file path.
 */
function ConvertExcelToCSV( $course_periods_import_file_path )
{
	$excel_extensions = [ '.csv', '.xls', '.xlsx' ];

	$file_ext = mb_strtolower( mb_strrchr( $course_periods_import_file_path, '.' ) );

	if ( ! in_array( $file_ext, $excel_extensions ) )
	{
		// Not an Excel file.
		return $course_periods_import_file_path;
	}


	if ( $file_ext === '.csv' )
	{
		$csv_file_path = $course_periods_import_file_path;
	...
	{
		// Error PHP zip extension required to open .xlsx or .ods file.
		$error = [
			dgettext( 'Timetable_Import', 'PHP zip extension is required. Please enable it or convert your file to CSV.' )
		];

		ErrorMessage( $error, 'fatal' );
	}

	require_once 'modules/Timetable_Import/classes/PHPExcel/IOFactory.php';

	$objPHPExcel = PHPExcel_IOFactory::load( $course_periods_import_file_path );

	$loadedSheetNames = $objPHPExcel->getSheetNames();

	$objWriter = PHPExcel_IOFactory::createWriter( $objPHPExcel, 'CSV' );

	unlink( $course_periods_import_file_path );

	$csv_file_path = mb_substr(
		$course_periods_import_file_path,
		0,
		mb_strrpos( $course_periods_import_file_path, '.' )
	) . '.csv';
...
function _isNewCoursePeriod( $old_course_period, $course_period )
{
	...
	if ( $old_course_period['SUBJECT'] === $course_period['SUBJECT']
