...
function StudentIDCardGetCustomCSS()
{
	...
	if ( empty( $custom_css['photo_float'] )
		|| ! in_array( $custom_css['photo_float'], [ 'left', 'right' ] ) )
	{
...
function StudentIDCardOptionsFormHTML( $custom_css )
{
	...
	$html = '<tr class="st"><td>' .
		'<img src="' . URLEscape( $custom_css['background_image'] ) . '?cache_killer=' . rand() .
		'" style="max-width: calc( 324px * 1.5 ); max-height: calc( 204px * 1.5 );" /><br />' .
	FileInput(
		'background_image',
