...
function StudentQuizSubmissionOutput( $quiz_id, $student_id = 0 )
{
	...
	{
		echo '<br /><div class="center">' . SubmitButton( _( 'Submit Quiz' ), 'submit_quiz' ) . '</div>';
	}
...
function QuizQuestionIsAnswerCorrect( $question, $answer )
{
	...
	switch ( $question['TYPE'] )
	{
		...
		case 'gap':
			...
			// Compare answers, then in lower case.
			$correct = $answer_value === $question_answer
				|| mb_strtolower( $answer_value ) === mb_strtolower( $question_answer );
...
function QuizQuestionGapAnswerPoints( $question, $answer )
{
	...
	foreach ( $strings as $i => $string )
	{
		if ( $underscores_count++%2 )
		{
			...
			// Compare answers, then in lower case.
			if ( $answer_strings[ $i ] === $string
				|| mb_strtolower( $answer_strings[ $i ] ) === mb_strtolower( $string ) )
			{
				$correct++;
			}
		}
